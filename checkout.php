	...
	<div class="spacer"></div>
		<?php
			...
			if (strlen($queryName) > 0) {
				$query = file_get_contents("SQL/queries/query_".$queryName.".sql");
			}
						
			switch($queryName) {
				case "":
					echo "
						<h2 class='text-center white' style='margin-top:20px;margin-bottom:20px'>
							Check what other search for
						</h2>

						<div class='text-center' style='line-height:1.75;font-size:16px;margin-left:20px;'>
							<a href='checkout.php?query=j'>Average restaurant ratings</a><br>
						</div><hr>
						<div class='text-center' style='line-height:1.75; font-size:16px'>
							<a href='checkout.php?query=k'>Raters with the highest ratings</a><br>
						</div><hr>
					";
					break;
				case "c": default:
					...
					echo "	
						<h2 class='text-center white' style='margin-bottom:20px'>
					...
					while ($res = pg_fetch_array($result)) {
						...
						echo "
						<div  style='line-height:1.75;font-size:16px;margin-left:20px;'>
				...
				case "g":
					...
					echo "<select style='margin-bottom:20px;color:white;' class='center-block' id='monthDrop' name='monthDrop' onchange='changeDate()'>";
					$month = 1;
					while($month <= 12) {
						switch($month) {
							case 1: $monthName = "January"; break;
						...
						echo "<option style='color:black;' value='$month'";
					...
					echo "<select style='margin-bottom:20px;color:white;' class='center-block' id='yearDrop' name='yearDrop' onchange='changeDate()'>";
					$year = date("Y");
					while($year >= 1900) {
					...
					while($res = pg_fetch_array($result)) {
						...
						if ($isNotLast) {
							...
							echo "
								<div  style='line-height:1.75;font-size:16px;margin-left:20px;'>
									<strong><a href='restaurant.php?id=$locationId'>$restName</a></strong><br>
									<a href = 'cuisines.php?query=$description&cui=$description'>$description</a><br>
				...
				case "j":
					...
					while($res = pg_fetch_array($result)) {
						...
						echo "
						<div  style='line-height:1.75;font-size:16px;margin-left:20px;'>
							<strong><a href='cuisines.php?query=$cuisine&cui=$cuisine'>$cuisine: </a></strong>$avgRate<br>
						</div><hr>
						";
					}
					break;
				...
				case "m":
					...
					while($res = pg_fetch_array($result)) {
						...
						if (strcmp($lastUser, $userName) != 0) {
							...
							$userInfo = "<div  style='line-height:1.75;font-size:16px;margin-left:20px;'>
									<strong><a href='profile.php?name=$userName'>$userName</a></strong><br>
				...
				case 'p':
					...
					switch($extraOne) {
						case '25': $link = "<a href='popular.php?query=p&amp;extrao=50'>25</a>"; break;
						case '50': $link = "<a href='popular.php?query=p&amp;extrao=100'>50</a>"; break;
						case '100': $link = "<a href='popular.php?query=p&amp;extrao=25'>100</a>"; break;
					}
					echo "	
						<h2 class='text-center white' style='margin-bottom:20px'>
