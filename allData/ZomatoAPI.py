...
for i in range(0, 3):
    ...
    for rID in rIDs:
        ...
        for r in review['user_reviews']:
            ...
            varDate = randomDate()
            rating = int(r['review']['rating'])
            comment = r['review']['review_text'].replace("'", "")
            rating = int(math.ceil(r['review']['rating']))
            if rating == 0:
                rating = 1
            insertStatement = "INSERT INTO RATING(UserID, varDate, Price, Food, Mood, Staff, Comments, RestaurantID) VALUES ({},'{}',{},{},{},{},'{}',{});".format(uID, varDate, rating, randomRating(rating), randomRating(rating), randomRating(rating), comment, rID)
            
            itemReview.write(insertStatement+"\n")
...
def createAllTables(cuisineID, pyzomato):
    
    for k,v in cuisineID.items():
        result = pd.DataFrame(pyzomato.search(cuisines = k, q = "Ottawa"))
        
        
        for r in result['restaurants']:
            insertRestaurant(pyzomato, r['restaurant']['R']['res_id'], k,0)
createAllTables(cuisineID, pyzomato)
